...
procedure Simulation is
   ...
   task body Buffer is
      ...
      begin
         for W in Producer_Type loop
            Put_Line("|   Storage: " &
                     Integer'Image(Storage(W)) & " x " & Product_Name(W));
         end loop;
         Put_Line("|   Total items in storage: " &
                  Integer'Image(In_Storage));
      end Storage_Contents;

   begin
      Put_Line(ESC & "[91m" & "S: Storage started" & ESC & "[0m");
      Setup_Variables;
      loop
         accept Take(Product: in Producer_Type; Number: in Integer) do
            if Can_Accept(Product) then
               Put_Line(ESC & "[91m" & "S: Accepted item: " &
         ...
         accept Deliver(Assembly: in Assembly_Type; Number: out Integer) do
            if Can_Deliver(Assembly) then
               Put_Line(ESC & "[91m" & "S: Delivered " &
                        Assembly_Name(Assembly) & " no." &
